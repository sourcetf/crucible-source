...
impl SessionState {
    ...
    /// Return peer certificate chain as `Vec<Vec<u8>>` (DER, leaf first).
    /// Never panic — incomplete handshake yields `None`.
    #[inline]
    pub(crate) fn peer_identity(&self) -> Option<Box<dyn Any>> {
        ...
        if let Some(stack) = self.ssl.peer_cert_chain() {
            for cert in stack {
                if let Ok(der) = cert.to_der() {
                    if !der.is_empty() && !ders.iter().any(|d| d == &der) {
                        ders.push(der);
                    }
                }
            }
    ...
    pub(crate) fn early_crypto(
        ...
        let header_key = early_secret.header_key(version, suite).ok()?.as_crypto().ok()?;
        let packet_key = Box::new(early_secret.packet_key(version, suite).ok()?);
