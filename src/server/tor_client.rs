...
/// SOCKS5 握手（纯手写，无依赖）。
///
/// 目标地址按 RFC 1928 选择 ATYP：
///   域名 → 0x03（1 字节长度 + N 字节主机名，由代理侧解析，.onion 必须走这条）；
///   IPv4 → 0x01（4 字节）；
///   IPv6 → 0x04（16 字节）。
/// 早期实现把 IPv6 也塞进域名形式并直接报错，导致 v6 出站不可用。
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    // CONNECT：VER | CMD | RSV | ATYP | DST.ADDR | DST.PORT
    let mut req = vec![0x05u8, 0x01, 0x00];
    match target {
        TorTarget::Domain(h, _) => {
            let host_b = h.as_bytes();
            if host_b.len() > 255 {
    ...
    let port = match target {
        TorTarget::Domain(_, p) => *p,
        TorTarget::Ip(sa) => sa.port(),
    };
    ...
    if head[0] != 0x05 {
        bail!("socks5: reply bad version {:#x}", head[0]);
    }
    if head[1] != 0x00 {
    ...
    match head[3] {
        ...
        other => bail!("socks5: reply bad ATYP {other}"),
    }
    Ok(stream)
}
